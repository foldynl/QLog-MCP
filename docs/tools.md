...
Optional `calculations` derive numeric values from metric results without returning QSO rows to the caller. Each calculation names `left` and `right` inputs, an operation, and its returned `as` field. An input may exactly name a numeric metric or an earlier calculation in the same ordered list, or it may be a finite JSON number. Numeric literals are bound as SQLite parameters; booleans, numeric strings, non-finite numbers, group dimensions, forward references, arbitrary SQL, and general expressions are not accepted. Up to 20 calculations may use `add`, `subtract`, `multiply`, `divide`, or `percentage`. `divide` returns a floating-point ratio such as `0.75`; `percentage` returns that ratio multiplied by 100, such as `75.0`. A null input or zero denominator returns `null`, and no automatic rounding is applied.
